-- localization.lua
-- Cartographer 2D localisation-only configuration.
-- Used for the slam_localization profile (known map, no new mapping).
-- Load a saved .pbstream file to initialise against a pre-built map.
...
-- ── Pose graph — tighter for localisation only ────────────────────────────
...
-- In localisation mode we freeze the frozen trajectory (the loaded map)
-- and only optimise the new live trajectory against it.
